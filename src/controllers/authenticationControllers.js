...
const { userCreated, findByEmail, saveToken, UpdateUserByEmail } = require("../services/authenticationServices/user.service");
  ...
  try {
    ...
    jwt.verify(token, process.env.SECRET_KEY, async (err, decoded) => {
      ...
      try {
        ...
        const editPassword = await User.findOneAndUpdate({_id: id}, {password: hashedPassword}, {new: true});
        if(!editPassword){
